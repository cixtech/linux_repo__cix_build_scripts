...
prepare_debs() {
    ...
    if [[ "$DEBIAN_MODE" == "1" ]] && [[ "${DOCKER_MODE}" != "docker" ]]; then
        ...
        #cix_download -s "linux-weekly:cix_master-MPV%2Flatest/libmpv-dev_0.35.1-4_arm64.deb" -f ${PATH_DEB}/libmpv-dev_0.35.1-4_arm64.deb
        cix_download -s "linux-weekly:cix_master-MPV%2Flatest/libmpv2_0.35.1-4_arm64.deb" -f ${PATH_DEB}/libmpv2_0.35.1-4_arm64.deb
        cix_download -s "linux-weekly:cix_master-MPV%2Flatest/mpv_0.35.1-4_arm64.deb" -f ${PATH_DEB}/mpv_0.35.1-4_arm64.deb
    fi
...
"
    ...
    preseed="d-i pkgsel/include string smplayer chromium ffmpeg power-profiles-daemon perl memtester bonnie++ alsa-ucm-conf mpich libfdk-aac2 fprintd glmark2-x11 glmark2-wayland glmark2-es2-x11 glmark2-es2-wayland glmark2-es2-drm glmark2-drm iperf3 lm-sensors libdrm-etnaviv1 libdrm-freedreno1 libdrm-tegra0 libva-glx2 libwayland-bin v4l-utils gstreamer1.0-tools libatlas3-base firmware-amd-graphics firmware-misc-nonfree firmware-linux-nonfree"
    ...
    cd $new_files/pool/main/c/chromium
    rm -f chromium*.deb
    cd -
    ...
    echo "${preseed}" >> "${PATH_ROOT}/debian12/script/preseed.cfg"
    local preseed2="
d-i preseed/late_command string in-target apt-get -y remove switcheroo-control mesa-vulkan-drivers;in-target apt-get -y autoremove;in-target update-initramfs -c -k ${linux_version} -b /boot"
    ...
    if [[ "$ACPI" == "0" ]]; then
        ...
        if [[ -e "${new_files}/install.a64/initrd.gz" ]]; then
            ...
            sudo cp -rf ${PATH_OUT}/linux-image/lib/modules/* ./lib/modules/
            sudo rm -rf ./lib/modules/${linux_version}/extra/*
            sudo rm -rf ./lib/modules/${linux_version}/kernel/drivers/gpu
            if [[ -e "${HOME}/cix_fb" ]]; then
                sudo cp "${HOME}/cix_fb" ./
            fi
            sudo find . | sudo cpio --quiet -o -H newc > "${new_files}/install.a64/initrd"
...
do_build() {
    ...
    if [[ ! -e "${PATH_DEBIAN}" ]]; then
        ...
        ("1") #gnome + xfce
            ...
            if [[ -e "${PATH_ROOT}/build-scripts/debian/growroot.sh" ]]; then
            ...
            if [[ -e "${PATH_ROOT}/build-scripts/debian/growroot_hook" ]]; then
        ...
        ("5") #Openkylin V2.0 Release
	        ...
	        sudo cp -rf ${PATH_ROOT}/component/cix_opensource/npu/ai_demo/linux_basic/cix-stablediffusion-demo ${PATH_DEBIAN}/opt/cix/
...
update-initramfs -c -k ${linux_version} -b /boot
    "
        ...
        sudo cp "${PATH_ROOT}/tmp/install.sh" "${PATH_DEBIAN}/"
	      if [[ "$DEBIAN_MODE" != "1" ]]; then  #Running update-initramfs in OpenKylin will hang.
		        sudo sed -i '/update-initramfs/d' "${PATH_DEBIAN}"/install.sh
	      fi
