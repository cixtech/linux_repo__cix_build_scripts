...
cross_env() {
    ...
    echo "c_link_args = ['--sysroot=${PATH_SYSROOT}', '-L${PATH_SYSROOT}/usr/lib/aarch64-linux-gnu', '-L${PATH_SYSROOT}/usr/lib', '-Wl,-rpath-link,${PATH_SYSROOT}/usr/lib/aarch64-linux-gnu', '-Wl,-rpath-link,${PATH_SYSROOT}/usr/lib']" >> cross_env.txt
}
...
do_build() {
    ...
    local path_gst_cix="${PATH_ROOT}/component/cix_opensource/gstreamer-cix"
    if [[ -e "${path_gst_cix}" ]]; then
        if [[ -e "${path_gst}" ]]; then
            # gstreamer folder integrity check. If failed, re-clone it and apply patches.
            if [[ ! -e "${path_gst}/subprojects/gst-plugins-cix" ]]; then
                rm -rf "${path_gst}"
            fi
        fi
        if [[ ! -e "${path_gst}" ]]; then
            git clone -b 1.22.1 --depth=1 https://gitlab.freedesktop.org/gstreamer/gstreamer.git "${path_gst}"
            cd "${path_gst}"
            git apply "${path_gst_cix}/debian/gstreamer-1.22.1/patches/gstreamer_1_22_1_for_cix_2025q4.patch"
            cd -
            if [[ ! -e "${path_gst}/subprojects/gst-plugins-cix" ]]; then
                echo "gstreamer folder integrity check failed after patching, exit build."
                return
            fi
        fi
    fi
...
do_clean() {
    ...
    path_gst="${PATH_ROOT}/component/cix_opensource/gstreamer/"
    if [[ -e "${PATH_ROOT}/component/cix_opensource/gstreamer-cix" ]]; then
        rm -rf "${path_gst}"
        return
    fi
